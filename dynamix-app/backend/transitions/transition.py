...
def calculate_8bar_starts(bpm, track_length_ms, drop_ms):
    ...
    start_times = [
        drop_ms - (i * section_duration_ms)
        for i in range(int(drop_ms / section_duration_ms) + 1)
    ]
    start_times += [
        drop_ms + (i * section_duration_ms)
        for i in range(1, int((track_length_ms - drop_ms) / section_duration_ms) + 1)
    ]
    return sorted([time for time in start_times if 0 <= time <= track_length_ms])


def plot_detailed_waveforms(
    track1_samples,
    track2_samples,
    filtered_samples,
    final_samples,
    ...
    downsampled_length = (
        lambda samples: (len(samples) // downsample_rate) * downsample_rate
    )  # Ensure divisibility
    time_axis = lambda samples: np.linspace(
        0,
        downsampled_length(samples) / sample_rate,
        num=downsampled_length(samples) // downsample_rate,
    )
    ...
    for i, samples in enumerate(
        [track1_samples, track2_samples, filtered_samples, final_samples]
    ):
        downsampled_samples = samples[
            : downsampled_length(samples) : downsample_rate
        ]  # Trim to a divisible length before downsampling

        plt.subplot(4, 1, i + 1)
        plt.plot(time_axis(samples), downsampled_samples, label=titles[i])
        plt.axvline(
            x=transition_start_ms / 1000,
            color="red",
            linestyle="--",
            label=f"Transition Start @ {transition_start_ms / 1000}s",
        )
        plt.title(titles[i])
        plt.xlabel("Time (s)")
...
def gradual_high_pass_blend_transition(
    track1, track2, start_ms1, start_ms2, bpm1, bpm2
):
    ...
    for i in range(steps):
        ...
        if i >= steps // 2:
            ...
            incoming_segment = blend_segment[
                blend_current_ms : blend_current_ms + blend_step_ms
            ]
            ...
            volume_increase_db = (
                20 * np.log10(fade_in_factor) if fade_in_factor > 0 else -120
            )

            # Apply a low-pass filter that gradually increases in frequency to normal
            current_low_pass_freq = max_low_pass_freq - (
                max_low_pass_freq - min_low_pass_freq
            ) * (
                blend_index / blend_steps
            )  # Gradually approach 500 Hz
            ...
            incoming_blend = incoming_blend.overlay(
                incoming_segment, position=blend_current_ms
            )

    # Combine filtered and incoming blend segments
    filtered_segment = filtered_segment.overlay(
        incoming_blend, position=transition_duration_ms // 2
    )
...
def main(
    ...
    if transition == "blend1":
        transitioned_track, filtered_segment = gradual_high_pass_blend_transition(
            track1, track2, beat_drop_track1_ms, beat_drop_track2_ms, bpm1, bpm2
        )
        transitioned_track.export(
            f"tests/{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_gradual_high_pass_blend_transition_output.wav",
            format="wav",
        )
        filtered_segment.export(
            f"tests/{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_gradual_high_pass_blend_transition_filtered.wav",
            format="wav",
        )
